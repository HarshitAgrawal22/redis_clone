...
class LISTS_TASKS:
    ...
    def task_rpush_command(msg: Message, server):
        ic(msg.cmd.item)
        msg.conn_peer._list.rpush(msg.cmd.item)
        msg.conn_peer.socket_handler.send("OK","s")

    @staticmethod
    def task_lpush_command(msg: Message, server):
        ic(msg.cmd.item)
        msg.conn_peer._list.lpush(msg.cmd.item)
        msg.conn_peer.socket_handler.send("OK","s")

    @staticmethod
    def task_lpull_command(msg: Message, server):

        msg.conn_peer.socket_handler.send(f"{ msg.conn_peer._list.lpull()}","b")

    @staticmethod
    def task_rpull_command(msg: Message, server):

        msg.conn_peer.socket_handler.send(f"{ msg.conn_peer._list.rpull()}", "b")
